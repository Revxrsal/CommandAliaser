 ...
 */
package net.reflxction.commandaliaser.commands;
...
import net.reflxction.commandaliaser.gui.AliasGUI;
...
public class AliaserCommand implements ICommand {
    ...
    public String getCommandUsage(ICommandSender sender) {
        return "/commandaliaser <toggle / check / update / alias>";
    }
            ...
            case 1:
                switch (args[0]) {
                    case "toggle":
                        ...
                        break;
                    case "update":
                        if (CommandAliaser.INSTANCE.getChecker().isUpdateAvailable()) {
                            Multithreading.runAsync(() -> {
                                if (CommandAliaser.INSTANCE.getUpdateManager().updateMod()) {
                    ...
                    case "gui":
                    case "alias":
                        TimerUtils.runAfter(() -> Minecraft.getMinecraft().displayGuiScreen(new AliasGUI()), 50);
                        break;
                    default:
                        SimpleSender.send("&cIncorrect command usage. Try " + getCommandUsage(sender));
                        break;
